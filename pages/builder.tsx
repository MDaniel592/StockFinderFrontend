    ...
    return selectedSocket
      ? productLists[CATEGORIES.COOLER.categoryName]?.filter(
          (cooler: ProductInfo) => {
            ...
            const selectedCpu: ProductInfo = build[CATEGORIES.CPU.name]?.product
    ...
    return selectedSocket
      ? productLists[CATEGORIES.CHASSIS.categoryName]?.filter(
          (chassis: ProductInfo) => {
            ...
            const selectedMdb: ProductInfo = build[CATEGORIES.MDB.name]?.product
    ...
    <BuilderContext.Provider value={{ onProductSelected, onProductRemoved }}>
      <section className="default-w-space mt-4 lg:mt-0">
        ...
        <p className="text-sm my-2 text-justify w-full lg:w-3/4 xl:w-3/5 2xl:w-1/2 text-neutral-300">
          Bienvenido al Configurador de Equipos de StockFinder. En esta sección
          podrá tener al alcance del ratón los componentes de StockFinder para
          generar configuraciones de equipos que tengas en mente y exportarlos o
          guardarlos en tu perfil para más adelante.
        </p>
        <p className="text-sm my-2 text-justify w-full lg:w-3/4 xl:w-3/5 2xl:w-1/2 text-neutral-300">
          En base a los componentes elegidos en cada paso se filtrarán los de
          los siguientes. ¡Ya no tienes que preocuparte de si tu nueva RTX 4090
          TI no cabe en la torre!
        </p>
      </section>
      <section className="builder default-w-space">
        ...
        <article className="step">
          <div className="flex items-center gap-2">
            <label htmlFor="cpu-list-picker" className="text-md font-semibold">
        ...
        <article className="step hide-step">
          ...
          <p className="mt-2 text-xs w-full lg:w-3/4 xl:w-3/5 2xl:w-1/2 text-neutral-300">
            <span className="bg-error text-white p-1 rounded-md font-bold mr-1 text-xxs">
              ¡Por cierto!
            </span>
            Aunque hacemos algunas comprobaciones para que tu equipo tenga una
            configuración válida, asegúrate de que esto es así con un experto en
            caso de dudas. No nos hacemos responsables en el caso de que una
...
export async function getServerSideProps() {
  ...
  try {
    ...
    products.forEach(
      productList => (productsList[productList.category] = productList.products)
    )
  } catch (error) {
    return {}
  }
