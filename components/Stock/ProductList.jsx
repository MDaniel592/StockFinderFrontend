...
export default function ProductList({ data, maxTableList, failOverImage = "" }) {
  ...
  const new_data = data.sort(function (a, b) { return a.price - b.price });
    ...
    <div name='Products'>
      <ul>
        ...
        {display_data.map((value) => {
          ...
          const url = value["url"].indexOf("pccomponentes.com") !== -1 ? REFER + value["url"] : value["url"];
              ...
              <li className="hidden lg:block my-2 stock-text" >
                <div className="mx-4 px-4 grid md:grid-cols-7 items-center font-medium bg-zinc-700 hover:bg-zinc-600 hover:text-blue-500 rounded-xl">
                  ...
                  <div className="md:col-span-4">
                    <Link href={url} className="hover:text-blue-500 hover:underline hover:decoration-blue-500" target="_blank">
                      <div className="flex items-center gap-2">
                        <img src={imageUrl} className=" h-10 w-10 rounded-md" />
                        {name.replace(/(\r\n|\n|\r)/gm, "")}
                      </div>
                    </Link>
                  </div>
                  <div className="text-right">{price}€</div>
                  <div className="md:col-span-2 text-right whitespace-nowrap">{value.shop}</div>
                ...
                <div className="bg-zinc-700 hover:bg-zinc-600 hover:text-blue-500 rounded-xl px-2 py-1">
                  <div className="flex gap-2 items-center">
                    ...
                    <div className="grid grid-rows-2 w-full items-center text-xs">
                      <Link href={url} target="_blank" className="hover:text-blue-500 hover:underline hover:decoration-blue-500">
                        <p className="leading-none"> {name.replace(/(\r\n|\n|\r)/gm, "")}</p>
                      </Link>
