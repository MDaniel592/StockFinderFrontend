...
export default function StockAlertForm({ updateAlert, gpuModel }) {
  ...
  return (
    <div className="my-2 text-center">
      <button onClick={handleButtonClick} className="btn-blue-white">
        {isFormVisible ? 'Cerrar' : 'Crear alerta'}
      </button>

      {isFormVisible && (
        <form
          onSubmit={handleSubmit}
          className="font-medium text-neutral-300 text-sm flex flex-col gap-2 mt-2 border-2 border-gray-600 rounded-lg p-2 bg-google-dark"
        >
          ...
          <button onClick={event => handleSubmit} className="btn-blue-white">
            Nueva Alerta
          </button>
        </form>
      )}
