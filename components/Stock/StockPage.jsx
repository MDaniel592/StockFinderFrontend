...
export default function StockPage({ data_recv, category }) {
  const no_data = (
    <section>
      <p className="text-center font-semibold text-base lg:text-xl font-medium text-red-500 lg:my-38"> En estos momentos NO hay {category} disponible</p>
    </section>
  );
  ...
  const [tableLen, setTableLen] = React.useState(Object.keys(data_recv).length - 4);
  const [maxTableList, setMaxTableList] = React.useState([0, 29]);

  // Price
  const defaultPriceValue = { min: data_recv["min_price"], max: data_recv["max_price"] }
  ...
  const handleButton = (name) => {
    switch (name) {
      ...
      case 'Estado':
        ...
        for (let value in statusList) { statusList[value] = !buttonStatus; if (buttonStatus === true && defaultStatusList[value]) { statusList[value] = true; } }
  ...
  function handleChange(name, value) {
    ...
    switch (name) {
      ...
      case 'Estado':
        ...
        let newStatuss = [];
        for (let value in statusPreferences) { if (statusPreferences[value]) { newStatuss.push(value); } }
  ...
  const data_list = [
    ...
    { name: "Estado", data: statusList, preferences: statusPreferences, handleChange: handleChange, handleButton: handleButton, button: buttonStatus, show_more: false }
  ];
  ...
  const TableHeaderSelectedValues = [{ selected: shopSelected, label: "Tiendas" }, { selected: statusSelected, label: "Estado" }];
  ...
  return (
    <section className="w-fit mx-auto">
      ...
      <div className="px-4 sm:px-6 text-white">
        <TableHeader data_values={TableHeaderDataValues} selectedValues={TableHeaderSelectedValues}
          tableLen={tableLen} handleSearch={handleSearch} removeFilters={removeFilters} />
      </div>

      <div className="flex-wrap bg-zinc-800 rounded-lg w-fit z-0 text-neutral-300 my-2 p-2">
        <ProductList data={filteredData} maxTableList={maxTableList} failOverImage="gpu" />
