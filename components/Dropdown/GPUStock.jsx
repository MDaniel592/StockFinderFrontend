...
export default function GPUStock({ generations }) {
  const defaultValues = { "RTX 4000": false, "RTX 3000": false, "RX 6000": false }
          ...
          <>
            <Popover.Button
              className={classNames(
                open ? "text-blue-500" : "",
                "group  rounded-md inline-flex items-center focus:outline-none hover:text-blue-500"
              )}
            >
            ...
            >
              <Popover.Panel className="absolute -left-10 sm:-left-0 flex bg-zinc-800 rounded-lg z-10">
                <div className="grid text-center">
                  ...
                  {generations.map((value) => {
                    ...
                    return (
                      <List dense key={lisLabelId}
                        onMouseEnter={() => handleShowingChange(value.name)}
                        onMouseLeave={() => handleShowingChange(value.name)}
                      >
                        <ListItem disablePadding key={labelId} className='hover:bg-zinc-600 rounded-xl'>
                          ...
                          <Popover className="relative">
                            <Popover.Button className='p-2 flex place-items-center'>
                              ...
                              <ListItemText primary={value.name} className="text-center w-max" />
                            </Popover.Button>
                            ...
                            >
                              ...
                              <Popover.Panel className="fixed top-0 left-full flex bg-zinc-800 rounded-lg z-10" onMouseEnter={() => handleShowingChange(value.name)}
                                onMouseLeave={() => handleShowingChange(value.name)}
                              >
                                <div className="grid text-center">
                                  <List dense>
                                    {value.values.map((second_value) => {
                                      ...
                                      const model = second_value.name.replaceAll('RX', '').replaceAll('RTX', '').replaceAll('TI', 'Ti')
                                      return (
                                        <ListItem disablePadding key={labelId} className='hover:bg-zinc-600 rounded-xl'>
                                          <ListItemButton href={second_value.href}>
                                            <ListItemAvatar>
                                              <Avatar alt={`Avatar n°${second_value + 1}`} src={second_value.img} />
                                            </ListItemAvatar>
