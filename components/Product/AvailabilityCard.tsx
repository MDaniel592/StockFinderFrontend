...
export default function AvailabilityCard({
  ...
  const REFER = ''
  const url =
    availability.url.indexOf('pccomponentes.com') !== -1
      ? REFER + availability.url
      : availability.url
  ...
  return (
    <li
      key={Math.random()}
      className="block mb-2 text-xs lg:text-md font-semibold"
    >
      <div className="text-center mx-1 lg:mx-4 grid grid-cols-4 lg:grid-cols-6 gap-4 items-center bg-zinc-700 hover:bg-zinc-600 hover:text-emerald-500 rounded-xl px-2 py-1">
        ...
        <div className="">
          {availability.stock ? (
            <span className="text-md text-success">Disponible</span>
          ) : (
        ...
        <div className="lg:col-span-1 ">
          {availability.stock ? (
            ...
            <Link
              ...
              target="_blank"
              rel="noreferrer"
              className="block p-1 lg:p-2 bg-zinc-200 outline outline-2 outline-gray-600 text-gray-600 hover:text-white hover:bg-zinc-500 rounded-md whitespace-nowrap"
            >
              Sin stock
            </Link>
          )}
