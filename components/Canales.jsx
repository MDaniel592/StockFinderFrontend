...
export default function Canales({ data }) {
  ...
  function CustomPill({ textToShow, type, property }) {
    ...
    const baseClassName =
      'inline-block px-1 rounded-full text-xxs lg:text-xs outline outline-1' +
      tempValue
    switch (type) {
      case SUCCESS:
        return (
          <span
            className={
              baseClassName + ' bg-success-light text-success outline-success'
            }
          >
            {textToShow}
          </span>
      ...
      case ERROR:
        return (
          <span
            className={
              baseClassName + ' bg-error-light text-error outline-error'
            }
          >
            {textToShow}
          </span>
  ...
  function EnhancedHead() {
    return (
      <div className="grid grid-cols-2 lg:grid-cols-3 border-b-2">
        <div
          className="font-semibold text-neutral-200 text-xxs lg:text-xs"
          key={Math.random()}
          align="left"
        >
        ...
        <div
          className="hidden lg:block font-semibold text-neutral-200 text-xxs lg:text-xs"
          key={Math.random()}
          align="center"
        >
  ...
  function getTable(data) {
    return Object.keys(data).map((key2, index2) => {
      ...
      let modelo = details.modelo
        .replaceAll('GeForce ', '')
        .replaceAll('Radeon ', '')
      let textColor = details.status ? 'text-red-500' : 'text-neutral-200'
      return (
        <div
          className={`bg-zinc-700 grid grid-cols-2 lg:grid-cols-3 rounded-md px-2 py-1 relativetext-neutral-200 lg:${textColor}`}
          key={Math.random()}
        >
          <div className="text-xxs lg:text-xs text-left">{modelo}</div>
          <div className="text-xxs lg:text-xs text-center">
            {details.max_price.toLocaleString('es-ES', {
              style: 'currency',
              currency: 'EUR'
            })}
          </div>
  ...
  function getCards(data, themeWithLocale) {
    return Object.keys(data).map(function (key, index) {
      ...
      return (
        <div
          className="bg-zinc-800 rounded-lg px-2 py-4 flex flex-col  gap-3 "
          key={Math.random()}
        >
          <div id="builder-section" className="h-[6vh] rounded-lg">
            <div className="flex items-center h-full">
              <h2 className="text-xl lg:text-2xl lg:text-3xl font-bold m-auto">
                {name}
              </h2>
            </div>
          </div>
    ...
    <section className="">
      ...
      <div id="CHAT" className="text-center py-3">
        <p className="text-xl mb-4">
          También, puede unirse a un chat grupal con cientos de usuarios:
        </p>
        <div className="flex gap-4 rounded-2xl py-4 px-6 bg-zinc-800 text-center w-fit mx-auto">
          <div className="m-auto shrink">
            <Image
              ...
              className="rounded-lg h-10 lg:h-14"
            />
          </div>
          <div className="m-auto">
            <p className="text-gray-200 text-xs lg:text-lg font-extrabold">
