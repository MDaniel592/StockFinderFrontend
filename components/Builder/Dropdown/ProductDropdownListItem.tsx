  ...
  function ProductDropdownListItem({
    ...
    const productAvailabilities = (
      <select
        ...
        value={
          selectedAvailability?.shopName ??
          product.availabilities[0].shopName
        }
      >
    ...
    return (
      <article className={`product my-auto md:my-0 grid grid-cols-1  md:flex mx-2 p-2 gap-1 md:gap-2 rounded bg-zinc-700 place-content-between ${selectedUUID === product.uuid ? "selected" : ""}`}>
        ...
        <div className="flex md:hidden justify-center">
          ...
          <div className="select-product mx-auto md:my-auto md:w-[14%] text-sm">
            <button
              className={`btn ${selectedUUID === product.uuid ? "secondary-extrasmall" : "primary-extrasmall"}`}
        ...
        <div className="hidden md:block select-product mx-auto md:my-auto md:w-[14%] text-sm">
          <button
            className={`btn ${selectedUUID === product.uuid ? "secondary-small" : "primary-small"}`}
            onClick={handleProductSelection}
            data-uuid={product.uuid}
          >
            {`${selectedUUID === product.uuid ? "Actualizar" : "Selecionar"}`}
