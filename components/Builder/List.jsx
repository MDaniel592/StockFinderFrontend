...
export default function List({ data }) {
    ...
    function getCompleteList() {
        ...
        for (const [key, value] of Object.entries(data)) {
            ...
            const imageUrl = value["image"] ? `https://images.stockfinder.tech${value["image"]}` : `/images/placeholder/${value["category"]}.webp`;
            let row = (
                <>
                    ...
                    <li key={idLabel} className="hover:bg-zinc-700 rounded-lg px-2">
                        ...
                        <div className='grid grid-cols-3 justify-between table-text-xxs'>
                            ...
                            <div className="flex-wrap text-center items-center">
                                <div>Actual</div>
                                <div className={`${text_color}`}>{value["actual_price"].toFixed(2)} € </div>
                            </div>
                ...
                <li key='final-prices' className="hover:bg-zinc-700 rounded-lg px-2">
                    <div className='grid grid-cols-3 justify-between table-text-xxs '>
                        ...
                        <div className="flex-wrap text-center text-xs items-center">
                            ...
                            <div className={`${total_actual_price > total_original_price ? 'text-red-500' : 'text-blue-500'}`}>{total_actual_price.toFixed(2)} €</div>
                        </div>

                        <div className="flex-wrap text-right text-xs items-end">
                            <div>Diferencia</div>
                            <div className={`${total_actual_price > total_original_price ? 'text-red-500' : 'text-blue-500'}`}>{total_actual_price > total_original_price ? '+' : ''}{(total_actual_price - total_original_price).toFixed(2)} €</div>
                        </div>
                    </div>
                </li>
            </>
    ...
    return (
        <div key="table-div-component-sdfsdfsf">
            <ul key="table-component-jidsnfids">
                {getCompleteList()}
            </ul>
        </div>
    );
