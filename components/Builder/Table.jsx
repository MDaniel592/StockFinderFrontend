              ...
              >
                {rows.map((row, index) => {
                  ...
                  const availability_url =
                    row['url'].indexOf('pccomponentes.com') !== -1
                      ? REFER + row['url']
                      : row['url']
                  const imageUrl = row['image']
                    ? `https://images.stockfinder.tech${row['image']}`
                    : `/images/placeholder/${row['category']}.webp`
                  let text_color = ''
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell
                        ...
                        padding="normal"
                      >
                        {row.original_price.toLocaleString('es-ES', {
                          style: 'currency',
                          currency: 'EUR'
                        })}
                      </TableCell>
                      <TableCell
                        className={`${text_color} table-text-xs`}
                        component="th"
                        id={`row-data-cell-actual_price-${index}`}
                        scope="row"
                        padding="normal"
                      >
                        {row.actual_price.toLocaleString('es-ES', {
                          style: 'currency',
                          currency: 'EUR'
                        })}
                      </TableCell>
                ...
                <TableRow
                  tabIndex="enhanced-table-row-price"
                  key="total_price"
                  sx={{ borderBottom: 'hidden' }}
                >
                  <TableCell></TableCell>
                  <TableCell className="text-neutral-300 font-semibold">
                    {total_original_price.toFixed(2) + ' €'}
                  </TableCell>
                  <TableCell
                    className={`${
