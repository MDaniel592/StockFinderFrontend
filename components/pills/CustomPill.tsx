...
export default function CustomPill({ textToShow, type, property }: { textToShow: string; type: string; property?: string }) {
  const tempValue = property ? property : ''
  const baseClassName = 'inline-block px-2 py-1 rounded-full text-xxs lg:text-xs outline outline-1 ' + tempValue
  switch (type) {
    case INFO:
