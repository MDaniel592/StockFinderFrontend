...
function descendingComparator(a, b, orderBy) {
  if (
    ...
    if (orderBy === "Tamaño Memoria" || orderBy === "Almacenamiento" || orderBy === 'Cantidad Memoria') {
...
export default function EnhancedTable({ data, headCells, failOverImage = "", categoryBuild = '' }) {




  const customBorder = { borderBottom: 1, borderBottomWidth: '1px', borderColor: '#AFAFAF' }
      ...
      <TableHead>
        <TableRow>
          {headCells.map((headCell) => (
            <TableCell
              className="table-text-xs hover:text-white"
              key={headCell.id}
  ...
  function getRemainingCells(row, labelId) {
    ...
    for (var item in headCells) {
      ...
      if (item_key === "price") item_data = item_data.toLocaleString("es-ES", { style: "currency", currency: "EUR" });

      let string_value = item_data
      if ((item_key === "Conexiones" || item_key === "Soporte Procesador") && item_data.length > 1) {
        string_value = ""
        if (typeof (item_data) === 'object') { for (index in item_data) string_value += item_data.at(index) + ", " }
      ...
      if (item_key === "price") {
        colum_data = (
          <TableCell sx={customBorder} key={labelItem} align="left" padding="normal" component="th" scope="row">
            <div className="flex flex-nowrap gap-2">
              <p className="table-text-xxs">{string_value}</p>
              {item_key === "price" && <div className={`h-2 w-2 rounded-lg ${color} my-auto`}></div>}
            </div>
          </TableCell>
        );
      } else {
  ...
  function addProduct2Build(productToBeAdded) {
    if (!productToBeAdded.category || productToBeAdded.category === '') return false
    if (productToBeAdded['Tipo Socket']) {
      let socket = productToBeAdded['Tipo Socket'].replace("Intel", "Socket").replace("AMD", "Socket");
      localStorage.setItem(SELECTED_SOCKET_KEY, socket);
    }

    const products = JSON.parse(localStorage.getItem(BUILD_IN_PROGRES_KEY) || "{}");
  ...
  return (
    <Box sx={{ width: "100%", backgroundColor: "transparent", boxShadow: "none" }}>
      <ThemeProvider theme={themeWithLocale}>
        <Paper sx={{ width: "100%", mb: 2, borderRadius: 3, backgroundColor: "transparent", boxShadow: "none" }}>
          <TableContainer>
            <Table
              ...
              <TableBody sx={{ backgroundColor: "transparent", boxShadow: "none" }}>
                ...
                {stableSort(rows, getComparator(order, orderBy))
                  ...
                  .map((row, index) => {
                    ...
                    const imageUrl = row["images"].length > 0 ? `https://images.stockfinder.tech${row["images"]}` : `/images/placeholder/${failOverImage}.webp`;
                    return (
                      <TableRow hover tabIndex={labelId} key={key}>
                        ...
                        >
                          <div className="flex">
                            ...
                            <Popover
                              ...
                              sx={{ pointerEvents: "none", margin: 1, bgcolor: 'transparent' }}
                              open={open}
                              anchorEl={anchorEl}
                              ...
                              elevation={0}
                            >
                              <img src={displayIMG} onError={onImageNotLoadedError} className="max-h-44	max-w-xs rounded-xl" />
                            </Popover>
                        ...
                        <TableCell sx={customBorder} className='text-center'>
                          <a type="button" className="btn-small cursor-pointer text-xxs" onClick={(e) => onButtonPressed(e, { ...row, 'category': categoryBuild })}>Añadir</a>
                        </TableCell>
          ...
          <TablePagination sx={{ backgroundColor: "transparent", boxShadow: "none" }}
