...
export default function TableHeader({
  ...
  function html_filters_version() {
    ...
    for (let item in data_values) {
      ...
      if (
        low_value == data_values[item]['default']['min'] &&
        high_value == data_values[item]['default']['max']
      )
        continue
      ...
      if (key === 'Almacenamiento' || key === 'Cantidad Memoria') {
        ...
        new_high_value =
          high_value < 40 ? new_high_value : new_high_value / 1024
      } else {
    ...
    for (let item in selectedValues) {
      let value = ''
      for (let option in selectedValues[item]['selected']) {
        if (selectedValues[item]['selected'][option])
          value += selectedValues[item]['selected'][option] + ' '
      }
      if (value) {
        let key = selectedValues[item]['label']
        labels.push({ [key]: value })
      }
    }
  ...
  return (
    <div
      name="tableheader"
      className="grid xl:flex justify-evenly xl:justify-between items-center gap-4"
    >
      <div className="mt-2">
        <span className="justify-start mt-3 font-bold text-blue-500">
          Productos disponibles: {tableLen}
        </span>
        <div>
          {
            <p className="font-medium">
              {Obj ? 'Filtros aplicados:' : 'Ningún filtro aplicado'}
            </p>
          }
        </div>
        ...
        <div className="mt-2 text-center sm:text-left">
          {Obj ? (
            <button className="btn-error" onClick={removeFilters}>
              Quitar Filtros
            </button>
          ) : (
            ''
      ...
      <div className="mt-4 lg:mt-0 z-0">
        <TextField
          ...
          label="Buscador"
          placeholder="Buscar"
          value={searchValue} // inputProps={{ "aria-label": "Buscar" }}
