    ...
    async function deleteAccountPopUp() {
        ...
        alert.fire({
            ...
            if (result.isConfirmed) {
                ...
                let rawResponse = await userService.deleteAccount({ ...values, token: cookieToken });
    ...
    return (
        <React.Fragment>
            <div className="px-4 text-sm text-center">
                <p className="text-xs my-2 font-sans font-semibold text-red-500 underline">Esta acción es irreversible</p>
                <input
                    ...
                    required
                />
                <p className="text-xxs lg:mx-4">Se eliminaran todos los datos relacionados con el usuario.</p>

                <ErrorMessageAlert hasError={errorMessage !== undefined} errorText={errorMessage}></ErrorMessageAlert>
                <button className="btn-error my-3 w-48 mx-auto" onClick={(e) => handleDeleteButtonClick(e)}>Eliminar cuenta</button>
