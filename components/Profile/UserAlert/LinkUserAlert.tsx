...
export default function LinkUserAlert({
  alert,
  userData
}: {
  ...
  if (userData.telegram)
    notificationTypes.push(
      new NotificationType(
        'Telegram',
        alert.productData.wantsToBeAlertedByTelegram
      )
    )
  ...
  const [productNotificationTypes, setProductNotificationTypes] =
    ...
    let response: Response = await userService.updateAlert(
      {
        ...
        emailAlert:
          productNotificationTypes.find(
            notificationType => notificationType.notificationName === 'Email'
          )?.wantsToBeNotifiedByThis ?? true,
        telegramAlert:
          productNotificationTypes.find(
            notificationType => notificationType.notificationName === 'Telegram'
          )?.wantsToBeNotifiedByThis ?? true
      },
      token,
      alertId
  ...
  function onProductNotificationChange(
    notificationTypeChanged: NotificationType
  ) {
    console.log('Se ha hecho llamado al cambio en el tipo de notificación')
    const index = productNotificationTypes.findIndex(
      notification =>
        notification.notificationName ===
        notificationTypeChanged.notificationName
    )
    if (index === -1) {
      console.error(
        'algo ha ido mal seteando los nuevos tipos de notificaciones'
      )
      return
    }
  ...
  const REFER = ''
  const product_url =
    alert.productData.productUrl.indexOf('pccomponentes.com') !== -1
      ? REFER + alert.productData.productUrl
      : alert.productData.productUrl

  return (
    <React.Fragment>
      <div className="relative grid items-center bg-zinc-800 rounded-xl pt-8 pb-2 px-2 text-neutral-300 w-60">
        <div className="absolute text-xxs italic top-2 left-2 text-white px-1 bg-blue-500 rounded-xl">
          {alert.alertType}
        </div>
        <button
          className="absolute w-6 top-2 right-2 hover:text-rose-600 text-white"
          onClick={deleteAlert}
        >
          <XIcon />
        </button>
        ...
        <div className="flex gap-2">
          ...
          <p className="text-xxs sm:text-xs font-semibold">
            <a
              href={product_url}
              className="hover:underline hover:underline-blue-500 hover:text-blue-500"
            >
        ...
        <div>
          <UserAlertInStock
            isInStock={alert.productData.isInStock}
          ></UserAlertInStock>
        </div>
        ...
        <div className="w-full">
          <div className="text-left">
            ...
            <div className="flex flex-wrap justify-start text-xxs sm:text-xs font-semibold items-center">
              <h3 className="text-xs font-bold text-center"> Tienda:</h3>
              <span className="whitespace-pre"> {alert.productData.shop}</span>
            </div>
          </div>
        </div>
        ...
        <div
          key="alerts"
          className="w-full my-1 text-left flex gap-8 items-center text-xs font-semibold"
        >
